...
import { Menu, X, Sun, Moon, ArrowRight, ChevronDown, BookOpen, Users, Globe, LayoutGrid } from "lucide-react";
...
export default function Navbar() {
    ...
    return (
        <header className={`fixed w-full top-0 z-50 transition-all duration-500 ease-in-out ${isScrolled ? "bg-background shadow-xl py-4 border-b border-foreground/5" : "bg-background/90 dark:bg-transparent backdrop-blur-md dark:backdrop-blur-none py-6"}`}>
            <div className="max-w-7xl mx-auto px-6 sm:px-12 flex justify-between items-center">
                ...
                <nav className="hidden md:flex items-center gap-8">
                    ...
                    <Link href="/donate" className="px-6 py-2.5 rounded-full bg-brand-gold text-brand-white font-semibold text-sm hover:brightness-110 hover:shadow-[0_4px_15px_rgba(212,175,55,0.4)] transition-all duration-300 transform hover:-translate-y-0.5">
                        Donate
                    </Link>
                </nav>
                ...
                {mobileMenuOpen && (
                    ...
                    >
                        ...
                        <Link href="/donate" onClick={() => setMobileMenuOpen(false)}
                            className="block mt-2 text-center px-6 py-4 rounded-xl bg-brand-gold text-white font-bold text-lg">
                            Donate Now
                        </Link>
                    </motion.div>
                )}
